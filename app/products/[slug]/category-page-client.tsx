...
import { ArrowLeft, MessageCircle, Phone, Search, X, Zap } from "lucide-react"
import Reveal from "@/components/animations/reveal"
import StaggerContainer, { StaggerItem } from "@/components/animations/stagger"
import ProductModal from "@/components/products/product-modal"
import { categories, products, slugToValue, type Product } from "@/lib/products-data"
    ...
    <div>
          ...
          <div className="text-center">
            ...
            <Reveal direction="up" delay={0.1}>
              <h1 className="text-3xl md:text-4xl font-bold text-slate-900 mb-3 flex items-center justify-center gap-3">
                {categoryValue === "trending" && <Zap className="w-7 h-7 text-amber-400" />}
                {category.label}
              </h1>
            </Reveal>
            <Reveal direction="up" delay={0.15}>
              <p className="text-slate-500 max-w-lg mx-auto text-sm">{category.description}</p>
            </Reveal>
          ...
          {filtered.length === 0 ? (
            <div className="text-center py-20">
              <p className="text-slate-400 text-sm mb-4">
                {search ? `No products matching "${search}"` : "No products listed in this category yet."}
              </p>
              {search ? (
                <button
                  onClick={() => setSearch("")}
                  ...
                  <StaggerItem key={product.id}>
                    ...
                    >
                      <div className="flex-1 min-w-0">
                        ...
                        {product.hsn && <p className="text-xs text-slate-400 mt-0.5">HSN: {product.hsn}</p>}
                      </div>
      ...
      <section className="py-12 bg-slate-50 border-t border-slate-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <Reveal direction="up">
            <h2 className="text-xl font-bold text-slate-900 mb-2">Need a bulk quote for {category.label}?</h2>
